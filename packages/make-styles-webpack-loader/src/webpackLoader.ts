...
import { transformSync, TransformResult } from './transformSync';
 ...
 */
function parseSourceMap(inputSourceMap: WebpackLoaderParams[1]) {
  try {
    return typeof inputSourceMap === 'string' ? JSON.parse(inputSourceMap) : inputSourceMap;
  } catch (err) {
    return undefined;
  }
}
